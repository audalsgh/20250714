...
CNN 흐름 : "입력 이미지" -> 커널(필터)를 슬라이딩하며 계층별로 국소 영역의 정보를 요약(컨볼루션 N회) -> 새로운 2차원 "피쳐맵 결과"<br>
-> 활성화함수 ReLU 적용 -> 풀링 -> 플래튼 -> "완전연결 층" -> Loss Function과 정규화로 오차평가 -> Optimizer -> 전체 데이터셋을 돌면서 반복 = Epoch마다 정규화.<br>
특징
...
| **Activation Function(활성화 함수)**          | 합성곱·완전연결 층의 선형 출력에 비선형성을 부여. (대표: **ReLU**, Sigmoid, Tanh 등)                                |
| **Pooling (풀링)**               | 특징 맵 크기 축소 및 위치 변동 강인성 부여<br>- **Max Pooling**: 영역 내 최대값<br>- **Average Pooling**: 영역 내 평균값 |
| **Flatten(1차원으로 평탄화)**                      | 다차원 feature map을 1차원 벡터로 변환                                                                            |
| **Fully Connected Layer(완전연결 층)**        | 평탄화된 벡터를 입력으로 받아 최종 클래스 점수나 회귀값을 예측                                                     |
